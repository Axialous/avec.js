...
Dans le moteur, ces littéraux sont évalués comme des valeurs natives JavaScript: `:)` / `:(` deviennent `true` / `false`, `:o` devient `null`, et la syntaxe reste propre au langage d'expressions.
